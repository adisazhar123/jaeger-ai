...
import { fetchTrace, fetchMultipleTraces, searchTraces, searchTraceByQuery } from '../actions/jaeger-api';
...
function searchTraceByQueryStarted(state, { meta }) {
  console.log('in searchTraceByQueryStarted', state, { meta });
  const { query } = meta;
  const search = {
    query,
    results: [],
...
function searchTraceByQueryDone(state, { meta, payload }) {
...
function searchTraceByQueryErred(state, { meta, payload }) {
  if (!_isEqual(state.search.query, meta.query)) {
    return state;
  }
...
function fetchSearchStarted(state, { meta }) {
  const { query } = meta;
  console.log('fetchSearchStarted query', query)
  const search = {
    query,
    results: [],
    state: fetchedState.LOADING,
...
export default handleActions(
  {
    ...
    [`${searchTraceByQuery}_PENDING`]: searchTraceByQueryStarted,
    [`${searchTraceByQuery}_FULFILLED`]: searchTraceByQueryDone,
    [`${searchTraceByQuery}_REJECTED`]: searchTraceByQueryErred,
  },
  initialState
);
